...
/* like perl - @temp = split(/\s+/,$buf); 
		 Last ptr is a null. */
/* x = perl_split(buf);  then you are responsible for freeing *x and x */
...
extern char* return_tag(char*); /* returns a ptr to the tag in a tagged word (the/dt), or NULL if not tagged.*/
